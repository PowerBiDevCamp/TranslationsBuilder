...
style="width:4.39623in;height:2.40442in" />
...
While Translations Builder supports these 43 languages, it is important
to understand that it only supports a single culture identifier for each
language. For example, you can add the language **Spanish \[es-ES\]** to
your PBIX project. But you cannot add the language Spanish with the
culture identifier for a different locale such as **es-MX** for Spanish
in Mexico.
...
\[es-ES\]**. However, you can still load reports with a cultural
identifier of **es-MX**. The DAX code generated by Translations Builder
to implement report label translation only uses the first part of the
culture identifier.
...
add the language as **French \[fr-FR\]**. For any project that requires
Portuguese, you must add the language as Portuguese **\[pt-PT\]** and so
on.
